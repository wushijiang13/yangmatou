<template>
    <qt-view class="search_keyboard" :style="{ width: keyboardWidth + 'px' }" @childFocus="childFocus"
        ...
        <qt-view class="search_keyboard_input_root" :focusable="false">
            ...
            <input class="search_keyboard_input_text" ref="inputRef" type="text" placeholder="输入影片名称搜索"
                :value="inputText" :focusable="false" @change="onInputChange" :disable="true" />
        ...
        <qt-view class="search_keyboard_input_option_btns">
            <search-btn :next-focus-names="{ down: 'grid_view' }" @click="clearBtnClick"
                ...
                :icon-normal="ic_search_input_clear" :icon-focus="ic_search_input_clear_focus"
                search-txt-class="btn_text" :font-size="30" text="清空" />

            <search-btn :next-focus-names="{ down: 'grid_view' }" @click="deleteBtnClick"
                search-btn-class="search_keyboard_option_btn" style="margin-left: 113px;" :icon-width="32"
                :icon-height="32" :icon-normal="ic_search_input_delete" :icon-focus="ic_search_input_delete_focus"
        ...
        <qt-grid-view class="search_keyboard_list" ref="grid_view" name="grid_view" :clipChildren="false"
            ...
            <qt-view :type="1" :focusable="true" :focusScale="1.1" class="search_keyboard_item" eventClick eventFocus
                ...
                <qt-text :duplicateParentState="true" :ellipsizeMode="2" gravity="center" :fontSize="40"
                    :focusable="false" class="search_keyboard_item_text" text="${text}" />
            </qt-view>
        </qt-grid-view>
...
export default defineComponent({
    ...
    setup(props, context) {
        const keyboardWidth = computed(() => 634)
