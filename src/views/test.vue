<template>
  <div class="page">
    ...
    <qt-grid-view class="grid_view" ref="gridViewRef" :clipChildren="false" :clipPadding="false"
      :enablePlaceholder="true" padding="0,0,0,20" :list-data="listData" :preloadNo="5" :loadMore="loadMore"
      :listenBoundEvent="true" :openPage="true" :spanCount="6" :loadingDecoration="{ left: 950, top: 20, bottom: 20 }"
      @item-click="onItemClick">
      <qt-view type="1" class="qt_list_item" :focusable="true" eventClick>
        <qt-text text="${text}" class="qt_list_item_text"></qt-text>
      </qt-view>
      <template v-slot:loading>
        <qt-loading-view :type="1002" name="loading" color="rgba(255,255,255,0.3)" style="height: 40px;width: 40px;"
          :focusable="false" />
      </template>
      <qt-text :type="1003" style="width: 1920px;height: 100px;" gravity="center" :focusable="false"
        text="已经到底啦"></qt-text>
    </qt-grid-view>
  </div>
</template>
